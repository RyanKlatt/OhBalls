...
local function onGravityCollision(event)
	...
	if collision.myName == "blueBall" then
	event.contact.isEnabled = true
	blueBall:applyLinearImpulse( -3, 0, blueBall.x, blueBall.y )
	end
	if collision.myName == "greenBall" then
	event.contact.isEnabled = true
	greenBall:applyLinearImpulse( -3, 0, greenBall.x, greenBall.y )
	end
...
function scene:createScene( event )
	...
	blueTarget.x, blueTarget.y = 15, 130
	...
	blueRedTarget.x, blueRedTarget.y = 15, 130
	...
	-- make a ball
	blueBall = display.newCircle( 430, 15, 8 )
	...
	group:insert( blueBall )
		...
		-- make a ball
	greenBall = display.newCircle( 400, 45, 8)
	...
	local staticPart3 = display.newImageRect( "staticPart1.png", 80, 10 )
	staticPart3.x, staticPart3.y = 15, 180
	staticPart3.rotation = 90
	physics.addBody( staticPart3, "static" ,{ density= 0.1, friction=0.1, bounce=.2 } )
	group:insert( staticPart3 )
	
	local staticPart4 = display.newImageRect( "staticPart1.png", 100, 10 )
	staticPart4.x, staticPart4.y = 15, 65
	staticPart4.rotation = 90
	physics.addBody( staticPart4, "static" ,{ density= 0.1, friction=0.1, bounce=.2 } )
	group:insert( staticPart4 )
	
	local redBall = display.newCircle( 240, 140, 10)
	redBall.strokeWidth = 3
	redBall:setStrokeColor( 128, 0, 0)
	redBall:setFillColor( 0, 0, 0)
	physics.addBody( redBall, "static", { density=10.0, friction=0.5, bounce=0, radius=10 } )
	group:insert( redBall )
	
	local redBall2 = display.newCircle( 180, 70, 10)
	...
	local redBall3 = display.newCircle( 300, 210, 10)
	...
	accelUp2.x, accelUp2.y = 370, 150
	accelUp2.rotation = 270
	physics.addBody( accelUp2, "static", {isSensor=true, radius = 11} )
	accelUp2:addEventListener("collision", onGravityCollision)
