...
local function onRightGravityCollision(event)
	...
	if collision.myName == "blueBall" then
	event.contact.isEnabled = true
	blueBall:applyLinearImpulse( 2.2, 0, blueBall.x, blueBall.y )
	end
	if collision.myName == "greenBall" then
	event.contact.isEnabled = true
	greenBall:applyLinearImpulse( 2.2, 0, greenBall.x, greenBall.y )
	end
...
function scene:createScene( event )
	...
	blueTarget.x, blueTarget.y = 30, 30
	physics.addBody( blueTarget, {isSensor=true, radius=10})
	blueTarget.gravityScale = 0.1
	...
	blueRedTarget.x, blueRedTarget.y = 30, 30
	physics.addBody( blueRedTarget,  {isSensor=true, radius=10})
	blueRedTarget.gravityScale = 0.1
	blueRedTarget.isVisible = false
	group:insert( blueRedTarget )
	
	greenTarget = display.newImageRect( "greenTarget.png", 20, 20 )
	greenTarget.x, greenTarget.y = 225, 240
	physics.addBody( greenTarget,   {isSensor=true, radius=10})
	greenTarget.gravityScale = -0.5
	...
	group:insert( blueBall )
		...
		-- make a ball
	greenBall = display.newCircle( 50, 140, 8)
	...
	smallLevelPart.x, smallLevelPart.y = 260, 100
	smallLevelPart.rotation = 140
	physics.addBody( smallLevelPart, "static", { density= 0.1, friction=0.1, bounce=1.8 } )
	smallLevelPart:addEventListener( "touch", onTouch )
	group:insert( smallLevelPart )
	
	local accelLeft = display.newImageRect( "redArrow.png", 25, 25 )
	accelLeft.x, accelLeft.y = 420, 140
	...
	local accelRight = display.newImageRect( "redArrow.png", 25, 25 )
	accelRight.x, accelRight.y = 50, 160
	...
	local levelPart = display.newRect( 200, 20, 80, 20)
