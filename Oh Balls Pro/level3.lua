...
local function onGravityCollision(event)
	...
	if collision.myName == "blueBall" then
	event.contact.isEnabled = true
	blueBall:applyLinearImpulse( 0, 1, blueBall.x, blueBall.y )
	end
	if collision.myName == "greenBall" then
	event.contact.isEnabled = true
	greenBall:applyLinearImpulse( 0, 1, greenBall.x, greenBall.y )
	end
...
function scene:createScene( event )
	...
	blueTarget.x, blueTarget.y = 30, 240
	...
	blueRedTarget.x, blueRedTarget.y = 30, 240
	...
	greenTarget.x, greenTarget.y = 430, 60
	...
	greenRedTarget.x, greenRedTarget.y = 430, 60
	physics.addBody( greenRedTarget, "static", {isSensor=true, radius=10})
	greenRedTarget.isVisible = false
	group:insert( greenRedTarget )
	
	accelDown = display.newImageRect( "accelUp.png", 25, 25 )
	accelDown.x, accelDown.y = 30, 70
	accelDown.rotation = 180
	physics.addBody( accelDown, "static", { isSensor=true, radius = 11} )
	accelDown:addEventListener("collision", onGravityCollision)
	accelDown:addEventListener( "touch", onTouch )
	group:insert( accelDown )
	
	accelDown2 = display.newImageRect( "accelUp.png", 25, 25 )
	accelDown2.x, accelDown2.y = 410, 210
	accelDown2.rotation = 180
	physics.addBody( accelDown2, "static", {isSensor=true, radius = 11} )
	accelDown2:addEventListener("collision", onGravityCollision)
	accelDown2:addEventListener( "touch", onTouch )
	group:insert( accelDown2 )
	
	-- make a ball
	blueBall = display.newCircle( 160, 240, 8 )
	...
	group:insert( blueBall )
		...
		-- make a ball
	greenBall = display.newCircle( 320, 240, 8)
	...
	redBall = display.newCircle( 240, 140, 100)
	redBall.strokeWidth = 5
	redBall:setStrokeColor( 128, 0, 0)
	redBall:setFillColor( 0, 0, 0)
	physics.addBody( redBall, "static", { density=10.0, friction=0.5, bounce=0, radius=100 } )
